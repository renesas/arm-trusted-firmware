...
#if (APPLOAD == RZ_NOFIP)
#define FSP_FROM_XSPI_BASE (FSP_BASE - RZ_XSPI_MEMORY_BASE)
...
static void rza_xspi_read(void *to, uint64_t offset, size_t length)
{
	memcpy((void *)to, (void *)(RZ_XSPI_MEMORY_BASE + offset), length);
}
...
void rza_load_fsp(void)
{
	...
	rza_xspi_read(header, FSP_FROM_XSPI_BASE, sizeof(*header));

	ret = rz_check_fsp_header(header);

	if (ret == 0) {
		if ((FSP_BASE + RZ_APP_PAYLOAD_OFFSET) != header->dest_addr) {
			rza_xspi_read((void *)header->dest_addr,
